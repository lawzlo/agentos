...
test("watch rules trigger deduped tasks and can be enabled or disabled", async () => {
    ...
    try {
        ...
        const disabledPayload = await (await fetch(`${server.baseUrl}/watches/${watch.id}/disable`, { method: "POST" })).json();
        assert.equal(disabledPayload.watch.enabled, false);
        assert.equal(disabledPayload.watch.status, "disabled");
        const enabledPayload = await (await fetch(`${server.baseUrl}/watches/${watch.id}/enable`, { method: "POST" })).json();
...
test("cli can teach a completed task into a watch rule", async () => {
    ...
    try {
        ...
        const teachResult = await execFileAsync(process.execPath, [
            "bin/agentos.js",
            "watch",
            "teach",
            task.id,
