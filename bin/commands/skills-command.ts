...
export async function commandSkills(subcommand: string | undefined, positionals: string[], options: Record<string, any>) {
  if (subcommand === "ls") {
    ...
    console.log(payload.skills.map((skill: Record<string, any>) => `${skill.name}  ${skill.surfaceScope}`).join("\n") || "No skills found.");
    return;
  }
