...
`;
    ...
    if (process.platform === "win32") {
        print(options.json
            ...
            : "Windows auto-start is configured through Task Scheduler. Run the generated command manually on Windows.", options);
        return;
    }
...
async function daemonUninstall(options) {
    ...
    if (process.platform === "win32") {
        print(options.json ? { removed: true, command: "schtasks /Delete /TN AgentOS /F" } : "Remove the Task Scheduler entry on Windows with: schtasks /Delete /TN AgentOS /F", options);
        return;
    }
...
async function commandLogs(taskId, options) {
    ...
    console.log(events.map((event) => `${event.createdAt}  ${event.role}/${event.type}  ${event.message}`).join("\n") ||
        "No trace events yet.");
}
...
async function main() {
    ...
    if (!command || command === "help" || command === "--help") {
        print(`agentos daemon start|stop|status|logs|install|uninstall
...
agentos skills ls|inspect|run`, sharedOptions);
        return;
    }
    if (command === "daemon") {
        if (subcommand === "start") {
