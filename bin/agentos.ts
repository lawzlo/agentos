...
agentos watch add "<goal>" [--pack live-pack] [--skill name] [--workspace name] [--input key=value] [--approval auto|draft_only|confirm_required|blocked] [--reply-policy pack_default|auto_send|draft_first|prefill_first|approve_once_then_auto|blocked] [--reply-approval-window-ms n] [--cooldown-ms n] [--max-auto-actions-per-day n] [--max-consecutive-failures n] [--quiet-hours 22-8]
agentos watch teach <task-id> "<goal>" [--watch id] [--pack live-pack] [--workspace name] [--approval auto|draft_only|confirm_required|blocked] [--reply-policy pack_default|auto_send|draft_first|prefill_first|approve_once_then_auto|blocked]
...
async function main() {
  ...
  if (command === "desktop") {
    await commandDesktop(subcommand, positionals, sharedOptions);
    return;
  }
