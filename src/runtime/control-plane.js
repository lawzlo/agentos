...
export class ControlPlane {
    ...
    saveTaskAsWatchRule(taskId, { watchRuleId = null, goal = null, preferredSurface = null, workspaceName = null, livePack = null, appTarget = null, skillName = null, pollIntervalMs = null, enabled = null, triggerTexts = [] } = {}) {
        const task = this.store.getTask(taskId);
        if (!task) {
            throw new Error(`Task not found: ${taskId}`);
        }
        ...
        const existingWatchRule = (watchRuleId ? this.store.getWatchRule(watchRuleId) : null) ??
    ...
    async createTaskFromWatchRule(watchRule, detection = {}) {
        ...
        const actionTemplate = !watchRule.skillName && watchRule.watchProfile?.actionTemplate?.length
            ? materializeWatchActionTemplate(watchRule.watchProfile.actionTemplate, runtimeInputs, watchRule.watchProfile?.metadata?.templateInputs ?? [])
            : null;
        const taskSpec = detected.taskSpec ??
            {
                ...
                executionMode: actionTemplate?.length
                    ? "planned"
    ...
    async #restoreRuntimeState() {
        ...
        for (const task of interrupted) {
            ...
            if (updated?.traceId) {
                this.traceStore.finish(updated.traceId, "interrupted", "Daemon restarted before task completion.", this.#mergePersistedResult(task.id, { details: { reason: "daemon_restart" } }));
            }
        ...
        try {
            ...
            for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
                try {
                    ...
                    this.eventBus.broadcast("task.updated", this.getTask(taskId));
                    let result;
                    const controlGate = async ({ phase, step }) => this.#waitForExecutionAccess({
                        taskId,
                        traceId: trace.id,
                        phase,
                        step
