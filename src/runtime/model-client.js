export class OpenAICompatibleModelClient {
    ...
    async planTask(taskSpec) {
        return this.#requestJson({
            ...
            systemPrompt: "You are the Planner agent of AgentOS. Produce a concise step plan for a local worker. Use browser or desktop surfaces.",
    ...
    async decideNextAction(payload) {
        return this.#requestJson({
            ...
            systemPrompt: "You are the Autonomy agent of AgentOS. Decide one safe next action at a time from the visible local observation. Return done=true only when the task is verifiably complete.",
            userPayload: payload,
            temperature: 0
        });
    }
