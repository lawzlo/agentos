...
function materializeSkillValue(value, runtimeInputs = {}, skillInputs = []) {
    ...
    if (value && typeof value === "object") {
        return Object.fromEntries(Object.entries(value).map(([key, entry]) => [key, materializeSkillValue(entry, runtimeInputs, skillInputs)]));
    }
    return value;
}
...
function heuristicPlan(taskSpec) {
    ...
    const defaultSurface = taskSpec.preferredSurface === "desktop" || (!inputs.startUrl && inputs.desktopApp)
        ? "desktop"
        : "browser";
...
export class PlannerAgent {
    ...
    async preview(taskSpec) {
        ...
        if (!result.steps.length) {
            throw new PlanningError("No executable plan could be derived. Add more detail or switch to Advanced JSON.", { goal: task.goal });
        }
        return {
            ...result,
    ...
    async plan(task, traceId) {
        const result = await this.#derivePlan(task);
        if (!result.steps.length) {
            throw new PlanningError("No executable plan could be derived. Provide task.steps or model configuration.", { goal: task.goal });
        }
        this.traceStore.log({
            traceId,
            taskId: task.id,
            role: "planner",
            type: "plan.generated",
            message: result.source === "requested_skill"
