...
function buildActionTemplate({ planSteps = [], executionSteps = [], manualTeachSteps = [] }) {
    const executionById = new Map(executionSteps
    ...
    return baseSteps.concat(manualTeachSteps.map((step) => ({
        label: step.label,
        surface: step.surface,
        action: step.action,
...
function collectWatchInputs(taskSpec = {}) {
...
function parameterizeValue(value, templateInputs = []) {
    ...
    if (value && typeof value === "object") {
        return Object.fromEntries(Object.entries(value).map(([key, entry]) => [key, parameterizeValue(entry, templateInputs)]));
    }
    return value;
}
export function materializeWatchValue(value, runtimeInputs = {}, templateInputs = []) {
    ...
    if (value && typeof value === "object") {
        return Object.fromEntries(Object.entries(value).map(([key, entry]) => [key, materializeWatchValue(entry, runtimeInputs, templateInputs)]));
    }
    return value;
}
...
export function deriveWatchProfileFromExecution({ goal, taskId, taskSpec = {}, planSteps = [], executionSteps = [], manualTeachSteps = [], manualCorrections = [], result = null, overrides = {} }) {
