...
export function deriveWatchProfileFromExecution({
  ...
  const triggerTexts = uniqueStrings([
    ...
    ...(taskSpec.inputs?.waitText ? [taskSpec.inputs.waitText] : []),
    ...(taskSpec.inputs?.clickTarget ? [taskSpec.inputs.clickTarget] : []),
    ...(taskSpec.inputs?.watchItemText ? [taskSpec.inputs.watchItemText] : []),
    ...(taskSpec.inputs?.watchSummary ? [taskSpec.inputs.watchSummary] : []),
