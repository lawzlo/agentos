...
function parameterizeValue(value, skillInputs = []) {
    ...
    if (value && typeof value === "object") {
        return Object.fromEntries(Object.entries(value).map(([key, entry]) => [key, parameterizeValue(entry, skillInputs)]));
    }
    return value;
}
export class SkillRegistry {
    ...
    saveExecutionAsSkill({ name, surfaceScope, goal, result, taskId, taskSpec = {}, planSteps = [], executionSteps = [], manualCorrections = [], manualTeachSteps = [] }) {
        const taskInputs = (taskSpec.inputs ?? {});
        const rawActionTemplate = buildActionTemplate({ planSteps, executionSteps }).concat(manualTeachSteps.map((step) => ({
            label: step.label,
            surface: step.surface,
            action: step.action,
        ...
        const normalizedSurfaceScope = surfaceScope && surfaceScope !== "auto"
            ? surfaceScope
            : surfaces.length === 1
                ? surfaces[0]
                : "any";
