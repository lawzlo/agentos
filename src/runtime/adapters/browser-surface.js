...
export class BrowserSurfaceAdapter extends SurfaceAdapter {
  ...
  async verify({ workspace, expectation }) {
    ...
    if (expectation.selectorVisible) {
      const visible = await page.locator(expectation.selectorVisible).first().isVisible().catch(() => false);
    ...
    if (expectation.textVisible) {
      const visible = await page.getByText(expectation.textVisible, { exact: false }).first().isVisible().catch(() => false);
    ...
    if (expectation.targetVisible?.text) {
      const visible = await page.getByText(expectation.targetVisible.text, { exact: false }).first().isVisible().catch(() => false);
    ...
    if (expectation.selectorText) {
      const content = (await page.locator(expectation.selectorText.selector).textContent())?.trim() ?? "";
      details.selectorText = content;
      if (content !== expectation.selectorText.equals) {
        return { ok: false, details };
      }
    }
