...
export class DesktopSurfaceAdapter extends SurfaceAdapter {
  ...
  async verify({ task, workspace, traceId, expectation }) {
    ...
    if (expectation.fileExists) {
      try {
        await fs.access(expectation.fileExists);
    ...
    const targetText = expectation.textVisible ?? expectation.targetVisible?.text;
    if (targetText) {
