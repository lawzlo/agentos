...
export class ControlPlaneStore {
    ...
    `).run(record.id, record.goal, record.status, record.priority, record.triggerSource, record.deadline, record.preferredSurface, record.workspaceId, record.traceId, JSON.stringify(record.taskSpec), JSON.stringify(record.plan), JSON.stringify(record.result), record.error, record.createdAt, record.updatedAt);
    ...
    updateTask(id, patch) {
      ...
      WHERE id = ?
    `).run(merged.goal, merged.status, merged.priority, merged.triggerSource, merged.deadline, merged.preferredSurface, merged.workspaceId, merged.traceId ?? null, JSON.stringify(merged.taskSpec), JSON.stringify(merged.plan), JSON.stringify(merged.result), merged.error ?? null, merged.updatedAt, id);
    ...
    createTrace({ taskId, status = "running", plan = [] }) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `).run(row.id, row.taskId, row.status, row.startedAt, row.endedAt, row.summary, JSON.stringify(row.plan), JSON.stringify(row.output));
    ...
    updateTrace(id, patch) {
      ...
      WHERE id = ?
    `).run(merged.status, merged.endedAt ?? null, merged.summary ?? null, JSON.stringify(merged.plan), JSON.stringify(merged.output), id);
    ...
    appendTraceEvent(event) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(row.id, row.traceId, row.taskId, row.role, row.type, row.stepId, row.message, JSON.stringify(row.payload), row.createdAt);
        return row;
    }
    ...
    createWorkspace(workspace) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `).run(workspace.id, workspace.taskId, workspace.rootPath, workspace.profilePath, workspace.downloadsPath, workspace.artifactsPath, workspace.scratchPath, workspace.createdAt);
    ...
    `).run(row.id, row.name, row.rootPath, row.profilePath, row.downloadsPath, row.artifactsPath, row.scratchPath, JSON.stringify(row.metadata), row.createdAt, row.updatedAt);
    ...
    createArtifact(artifact) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `).run(row.id, row.taskId, row.traceId, row.kind, row.label, row.path, JSON.stringify(row.metadata), row.createdAt);
    ...
    putSkill(skill) {
      ...
      DO UPDATE SET
        ...
        metadata = excluded.metadata,
        updated_at = excluded.updated_at
    `).run(row.id, row.name, row.surfaceScope, JSON.stringify(row.triggerTerms), JSON.stringify(row.anchors), JSON.stringify(row.actionTemplate), JSON.stringify(row.successCriteria), JSON.stringify(row.recoveryHints), JSON.stringify(row.metadata), row.createdAt, row.updatedAt);
    ...
    putWatchRule(watchRule) {
      ...
      DO UPDATE SET
        ...
        updated_at = excluded.updated_at
    `).run(row.id, row.goal, row.enabled, row.status, row.preferredSurface, row.workspaceName, row.skillName, row.appTarget, row.livePack, row.pollIntervalMs, JSON.stringify(row.watchProfile), JSON.stringify(row.taskInputs), JSON.stringify(row.dedupeState), row.lastObservedAt, row.lastTriggeredAt, row.lastError, row.createdAt, row.updatedAt);
    ...
    putVaultEntry(entry) {
      ...
      DO UPDATE SET
        ...
        metadata = excluded.metadata,
        updated_at = excluded.updated_at
    `).run(row.id, row.scope, row.secretKey, row.ciphertext, row.iv, row.tag, JSON.stringify(row.metadata), row.createdAt, row.updatedAt);
