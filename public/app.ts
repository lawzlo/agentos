...
function byId(id: string): LooseElement {
  return document.getElementById(id) as LooseElement;
}
...
const exampleButtons = Array.from(document.querySelectorAll("[data-example]")) as LooseElement[];
...
function readFields(): Record<string, any> {
  return {
...
function renderTask(task) {
  ...
  (fragment.querySelector(".task-goal") as LooseElement).textContent = task.goal;
  (fragment.querySelector(".task-brief") as LooseElement).textContent = summarizeTask(task);
  (fragment.querySelector(".task-id") as LooseElement).textContent = `${task.id} · ${task.traceId ?? "no trace yet"}`;
  (fragment.querySelector(".task-status") as LooseElement).textContent = task.status;
  (fragment.querySelector(".task-result") as LooseElement).textContent = pretty(task.result ?? task.plan ?? {});

  const traceNode = fragment.querySelector(".task-trace") as LooseElement;
  ...
  const actionsNode = fragment.querySelector(".task-actions") as LooseElement;
...
function renderLive(tasks) {
  ...
  for (const task of tasks) {
    const fragment = liveTemplate.content.cloneNode(true) as DocumentFragment;
    (fragment.querySelector(".live-goal") as LooseElement).textContent = task.goal;
    const liveStatus = fragment.querySelector(".live-status") as LooseElement;
    liveStatus.textContent = `${task.status} · ${task.preferredSurface}`;

    const latest = task.trace?.events?.at(-1);
    const liveEvent = fragment.querySelector(".live-event") as LooseElement;
    ...
    const controls = fragment.querySelector(".live-controls") as LooseElement;
    ...
    if (["running", "planning", "verifying"].includes(task.status)) {
      controls.append(
        buildButton("暂停", "pause"),
        buildButton("接管", "request_takeover"),
        buildButton("停止", "stop")
      );
    } else if (task.status === "paused") {
      controls.append(
        buildButton("继续执行", "resume"),
        buildButton("接管", "request_takeover"),
        buildButton("停止", "stop")
      );
      ...
      controls.append(
        buildButton("记录修正并交还", "return_to_agent", "ghost-button", () => noteBox.value.trim()),
        buildButton("直接交还", "return_to_agent"),
        buildButton("停止", "stop")
      );
    }
...
function renderSkills() {
  ...
  for (const skill of learnedSkills) {
    const fragment = skillTemplate.content.cloneNode(true) as DocumentFragment;
    (fragment.querySelector(".skill-name") as LooseElement).textContent = skill.name;
    (fragment.querySelector(".skill-surface") as LooseElement).textContent = skill.surfaceScope;
    const inputs = skill.metadata?.skillInputs ?? [];
    (fragment.querySelector(".skill-brief") as LooseElement).textContent =
      inputs.length
    ...
    (fragment.querySelector(".skill-use") as LooseElement).addEventListener("click", () => {
      useSkill(skill);
    });
...
for (const input of Array.from(form.querySelectorAll("input, textarea, select")) as LooseElement[]) {
